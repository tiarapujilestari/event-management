...
export default function Checkout() {
  ...
  const checkoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        "Order placed! Please complete your payment from My Tickets.",
      );
      navigate("/orders");
    },
    onError: (err: any) =>
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        <p className="text-neutral-500">
          No items to checkout. Please select tickets from an event page first.
        </p>
        <button
      ...
      >
        ...
        <div className="mb-6 rounded-xl bg-neutral-50 dark:bg-neutral-800 p-4">
          ...
          <p className="text-sm text-neutral-500">
            {state.items.reduce((s, i) => s + i.quantity, 0)} ticket(s) selected
          </p>
        </div>
        ...
        <div className="mb-4">
          <label className="mb-1 flex items-center gap-2 text-sm font-medium">
            <Tag className="h-4 w-4" /> Referral Coupon
          </label>
          <input
            value={couponCode}
        ...
        <p className="mt-3 text-center text-xs text-neutral-400">
          You'll be able to upload your bank transfer proof from the My Tickets
          page.
        </p>
      </motion.div>
    </div>
