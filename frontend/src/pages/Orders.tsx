...
// Nicer labels than the raw enum names for the status badge
...
export default function Orders() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
              ...
              <div className="flex items-center justify-between border-t border-neutral-100 dark:border-neutral-800 pt-3">
                ...
                <div className="flex gap-2">
                  ...
                  {order.status === 'PENDING' && (
                    <button onClick={() => openPaymentModal(order)} className="btn-primary !px-3 !py-1.5 text-xs">
                      <Landmark className="h-3.5 w-3.5" /> Submit Payment
                    </button>
                  )}
                  {order.status === 'WAITING_CONFIRMATION' && (
                    <span className="text-xs text-neutral-400">Payment proof sent — waiting for admin review.</span>
                  )}
                </div>
              </div>
            </div>
      ...
      <Modal open={!!paymentOrder} onClose={closePaymentModal} title="Upload your payment proof">
        <p className="mb-4 text-sm text-neutral-500">
          Transfer <span className="font-semibold text-neutral-700 dark:text-neutral-200">{paymentOrder && formatCurrency(paymentOrder.total)}</span> to the bank account below, then upload a screenshot or photo of the transfer receipt. An admin will manually confirm it.
        </p>
        ...
        {bankInfo && (
          <div className="mb-5 rounded-xl bg-neutral-50 dark:bg-neutral-800 p-4">
            <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-brand-600">
              <Landmark className="h-4 w-4" /> TRANSFER YOUR PAYMENT TO THIS ACCOUNT
            </div>
        ...
        <label className="mb-2 block text-sm font-medium">Payment proof image</label>
        <input
          type="file"
          accept="image/*"
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {uploadProofMutation.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
            Upload proof
          </button>
        </div>
      </Modal>
