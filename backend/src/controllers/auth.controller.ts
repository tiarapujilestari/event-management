  ...
  const user = await prisma.$transaction(async (tx) => {
    ...
    if (referrer) {
      ...
      await tx.pointHistory.create({
        data: { userId: referrer.id, points: 10000, type: 'REFERRAL_BONUS', note: `Referral bonus from ${email}` },
      });
...
export const resetPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  await prisma.user.update({
    where: { id: user.id },
    data: { password: hashed, resetToken: null, resetTokenExpires: null, refreshToken: null },
  });
