...
export const organizerDashboard = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      ...
      data: {
        ...
        upcomingEvents,
        expiredEvents,
        revenueChart: Object.entries(revenueByMonth).map(
...
export const eventAttendees = asyncHandler(
  async (req: AuthRequest, res: Response) => {
...
export const exportAttendeesCsv = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const event = await prisma.event.findUnique({
      where: { id: req.params.eventId },
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="attendees-${event.slug}.csv"`,
    );
