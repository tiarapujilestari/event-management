...
// Wraps stock decrement + discount application + order creation in one SQL transaction
export const checkout = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    if (!event || event.status !== 'PUBLISHED') throw ApiError.badRequest('Event is not available for booking');

    let subtotal = 0;
    const orderItemsData = [];

    for (const item of items) {
      // Row lock via findUnique inside transaction + conditional update guards oversell
      ...
      if (remaining < item.quantity) {
        throw ApiError.badRequest(`Not enough seats available for "${ticketType.name}"`);
      }

      const lineTotal = Number(ticketType.price) * item.quantity;
      subtotal += lineTotal;

      // Atomically increment sold count; will fail validation above if oversold under concurrency
    ...
    // Voucher (organizer-created, event-specific)
    if (voucherCode) {
      const voucher = await tx.voucher.findUnique({ where: { code: voucherCode } });
      if (!voucher || voucher.eventId !== eventId) throw ApiError.badRequest('Invalid voucher code');
      if (voucher.usedCount >= voucher.usageLimit) throw ApiError.badRequest('Voucher usage limit reached');
      ...
      if (Number(voucher.minPurchase) > subtotal) {
        throw ApiError.badRequest(`Minimum purchase of ${voucher.minPurchase} required for this voucher`);
      }
    ...
    // Referral coupon
    if (couponCode) {
      const coupon = await tx.coupon.findFirst({ where: { code: couponCode, userId } });
    ...
    // Points redemption (1 point = 1 currency unit, capped at remaining total)
    let pointsUsed = 0;
    if (pointsToUse > 0) {
      const points = await tx.referralPoint.findMany({
        where: { ownerId: userId, isExpired: false, expiresAt: { gt: new Date() } },
      });
      ...
      if (pointsUsed > 0) {
        ...
        await tx.pointHistory.create({
          data: { userId, points: -pointsUsed, type: 'REDEEMED', note: `Used on order for event ${event.title}` },
        });
        // Deduct from oldest points first (simplified: just track via history; balance derived from sum)
        await tx.referralPoint.create({
          data: { ownerId: userId, points: -pointsUsed, expiresAt: new Date() },
        });
      }
    }
  ...
  res.status(201).json({ success: true, data: order, message: 'Order created. Proceed to payment.' });
});
...
export const myOrders = asyncHandler(async (req: AuthRequest, res: Response) => {
  const orders = await prisma.order.findMany({
    ...
    include: {
      event: { select: { title: true, slug: true, bannerUrl: true, startDate: true } },
